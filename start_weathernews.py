...
if len(list_comment) == 0:
	...
	job.enable()
	job2 = cron.new(command='find /home/pi/ -name "Assistant_main.log" -type f -size +5G -exec truncate -s 0 {} \;')
	job2.minute.every(6)
	job2.set_comment("Checks if the log size is higher than 5Go and truncates it to avoid space limitation")
	job2.enable()
	cron.write()
	time.sleep(1)
	now = datetime.now()
